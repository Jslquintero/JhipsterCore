...
In ./src/JhipsterCore/ClientApp run
...
auto-refreshes when files change on your hard drive.
    ...
    dotnet run --verbosity normal --project ./src/JhipsterCore/JhipsterCore.csproj
    npm --prefix ./src/JhipsterCore/ClientApp start

npm is also used to manage CSS and JavaScript dependencies used in this application. You can upgrade dependencies by
specifying a newer version in [package.json](package.json). You can also run `npm update` and `npm install` to manage dependencies.
Add the `help` flag on any command to see how you can use it. For example, `npm help update`.

The `npm --prefix ./src/JhipsterCore/ClientApp run` command will list all of the scripts available to run for this project.

### Managing dependencies

For example, to add [Leaflet][] library as a runtime dependency of your application, you would run following command:

    npm --prefix ./src/JhipsterCore/ClientApp install --save --save-exact leaflet

To benefit from TypeScript type definitions from [DefinitelyTyped][] repository in development, you would run following command:

    npm --prefix ./src/JhipsterCore/ClientApp install --save-dev --save-exact @types/leaflet
...
To build the artifacts and optimize the JhipsterCore application for production, run:

    cd ./src/JhipsterCore
    rm -rf ./src/JhipsterCore/wwwroot
    dotnet publish --verbosity normal -c Release -o ./app/out ./JhipsterCore.csproj

The `./src/JhipsterCore/app/out` directory will contain your application dll and its depedencies.
...
3. Run `` dotnet sonarscanner begin /d:sonar.login=admin /d:sonar.password=admin /k:"JhipsterCore" /d:sonar.host.url="http://localhost:9001" /s:"`pwd`/SonarQube.Analysis.xml" ``
...
```bash
docker build -f ./Dockerfile-Back -t jhipstercore .
```

Then run:

```bash
docker run -p 8080:80 jhipstercore
```
